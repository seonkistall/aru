...
세션의 네트워크에서는 aihub.or.kr 접속이 차단되어 파이프라인이 71645 페이지의
이용약관을 직접 읽을 수 없었다.

**2026-09-14 갱신**: 저장소 소유자가 71645 페이지의 이용약관을 직접 확인하고
**상업 학습 허용**으로 보고했다. `licenseTier`를 `commercial_ok`로 올렸고
`ml/licensing.py`는 이제 이 소스로 shipping 학습을 허용한다.

성격을 정확히 적어 둔다. 이건 **소유자의 확인 기록이지 파이프라인이 읽고 보관한
라이선스 문서가 아니다.** 인용할 약관 원문이 이 저장소에 없다. `licenseEvidence`
필드가 그 사실을 명시하고, 게이트가 결정을 출력할 때 함께 표시한다. 데이터 신청이
승인되면 **승인서에 붙는 조건을 다시 확인**하고 필요하면 tier를 내릴 것.

## 축별 커버리지 (현재 확정분)

| 축 | 퍼블릭 소스 | 상업 학습 | 실질 판정 |
|---|---|---|---|
| 모공 | AI-Hub 71645 (6등급) | **허용** (소유자 확인) | 신청 통과 시 유일한 감독 신호 |
| 주름 | AI-Hub 71645 (7등급, 3부위) | **허용** (소유자 확인) | 동일. FFHQ-Wrinkle은 비상업 |
| 색소침착 | AI-Hub 71645 (6등급, 2부위) | **허용** (소유자 확인) | 동일 |
...
| 수분 | AI-Hub 71645 기기 측정 | **허용** (소유자 확인) | 카메라 축으로 쓰지 않음 |
| 민감 | **없음** | — | 카메라 축이 아님 (설문) |
...
이 문서의 결론은 "5개 축은 자체 수집 외에 방법이 없다"였다. 그건 **분류 모델을
학습한다고 가정했을 때** 맞는 말이다. 홍조·유분·색감·트러블은 학습 대상이 아니라
같은 사진 안의 상대 측정으로 얻을 수 있고, 그 경우 데이터셋도 라이선스도 필요 없다.

근거와 설계는 [label-free-axes.md](label-free-axes.md)에 있다. 요약하면, 같은
얼굴을 세 기기로 찍은 벤치마크에서 **기기가 색 분산의 η²=0.1165를 설명하는 반면
피험자의 피부 상태는 η²=0.0006에 불과하고 유의하지도 않았다.** 절대 색으로
피부를 맞히는 모델은 카메라를 학습한다. 부위 간 상대값은 그 항이 상쇄된다.
...
- **71645의 이용약관 원문**. 소유자가 2026-09-14에 확인하고 상업 학습 허용으로
  보고했으나, 파이프라인이 직접 읽어 보관한 문서는 아니다(네트워크 차단). 이
  조사에서 기계가 실제로 읽은 라이선스 문서는 저장소의 MIT LICENSE(코드 전용)뿐이다.
  논문의 CC BY 4.0은 논문에 대한 것이지 이미지가 아니다.
