...
10. 환경 신호 4개(조명, T존 반사, 피부 영역, 노출 여유)를 확인합니다. 노출 여유는 볼
    패치에서 채널 하나라도 8비트 상한에 닿은 픽셀의 비율로, 붉은기·피부결 판독이
    바로 그 패치에서 계산되기 때문에 봅니다. 하나라도 실패하면 재촬영을 권합니다.
11. 속성 확신과 환경 신호를 결합해 confidence를 냅니다. 사용자에게 보이는 신뢰도
    라벨은 세 판독값이 경계에서 얼마나 떨어져 있는지를 알리고, 촬영 품질은 신호별
    체크리스트로 따로 보여 줍니다.
