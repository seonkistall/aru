...
> **이 파일은 2026-07-11 v1.1.0 사이클의 기록이다.** 2026-09-15부터 돌아가는 6시간
> 주기 자동 사이클의 프로토콜·가드레일·백로그·체인지로그는
> [`docs/AUTOPILOT.md`](docs/AUTOPILOT.md)에 있다. 두 파일이 같은 이름을 쓰는 건
> 사고였고(9월 사이클이 이 파일의 존재를 모르고 새로 만들었다), 실행 큐로 읽어야 할
> 것은 `docs/` 쪽이다.
