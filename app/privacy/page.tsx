    ...
    <main className="min-h-screen px-5 py-9" style={{ background: "var(--paper)" }}>
      <div className="mx-auto" style={{ maxWidth: 430 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
          ...
          <div role="group" aria-label={t("언어 선택 / Language")} style={segmented}>
        ...
        <h1 style={titleStyle}>{pick(t("사진 데이터는 목적별로 나눠서 다룹니다"), "We handle photo data separately, by purpose")}</h1>
        <p style={leadStyle}>
          {pick(
        ...
        <section style={sectionStyle}>
          <p style={sectionLabel}>{pick(t("기본 스캔"), "Default scan")}</p>
          <h2 style={sectionTitle}>{pick(t("기기 안에서 먼저 분석"), "Analyzed on your device first")}</h2>
          <p style={bodyText}>
            {pick(
        ...
        <section style={sectionStyle}>
          <p style={sectionLabel}>{pick(t("AI 분석용 전송"), "AI analysis transfer")}</p>
          <h2 style={sectionTitle}>{pick(t("선택한 경우에만 얼굴 크롭 전송"), "Face crops sent only if you opt in")}</h2>
          <p style={bodyText}>
            {pick(
        ...
        <section style={sectionStyle}>
          <p style={sectionLabel}>{pick(t("학습용 크롭 저장"), "Learning crop storage")}</p>
          <h2 style={sectionTitle}>{pick(t("연구 동의 샘플만 로컬 보관"), "Only research-consented samples, kept locally")}</h2>
          <p style={bodyText}>
            {pick(
        ...
        <section style={sectionStyle}>
          <p style={sectionLabel}>{pick(t("내보내기와 삭제"), "Export & delete")}</p>
          <h2 style={sectionTitle}>{pick(t("이 기기에 저장된 데이터"), "Data stored on this device")}</h2>
          <p style={bodyText}>
            {locale === "ko"
          ...
          <div style={{ display: "grid", gap: 8, marginTop: 14 }}>
            <button onClick={exportLabels} disabled={labelTotal === 0} style={{ ...outlineBtn, opacity: labelTotal === 0 ? 0.5 : 1 }}>{pick(t("라벨 JSONL 내보내기"), "Export labels (JSONL)")}</button>
            <button onClick={exportCropSamples} disabled={cropTotal === 0} style={{ ...outlineBtn, opacity: cropTotal === 0 ? 0.5 : 1 }}>{pick(t("크롭 JSONL 내보내기"), "Export crops (JSONL)")}</button>
            <button onClick={clearLearningData} style={dangerBtn}>{pick(t("로컬 학습 데이터 삭제"), "Delete local learning data")}</button>
          </div>
        </section>

        <section style={sectionStyle}>
          <p style={sectionLabel}>{pick(t("동의 기록"), "Consent log")}</p>
          <h2 style={sectionTitle}>{pick(t("분리된 동의 이력"), "Separate consent history")}</h2>
          <p style={bodyText}>
            {locale === "ko"
          ...
          <div style={{ display: "grid", gap: 8, marginTop: 14 }}>
            <button onClick={exportConsentEvents} disabled={consentTotal === 0} style={{ ...outlineBtn, opacity: consentTotal === 0 ? 0.5 : 1 }}>{pick(t("동의 기록 CSV 내보내기"), "Export consent log (CSV)")}</button>
            <button onClick={clearConsentLog} disabled={consentTotal === 0} style={{ ...dangerBtn, opacity: consentTotal === 0 ? 0.5 : 1 }}>{pick(t("동의 기록 삭제"), "Delete consent log")}</button>
          </div>
        </section>

        <section style={sectionStyle}>
          <p style={sectionLabel}>{pick(t("구매/상담 클릭 기록"), "Shopping & clinic click log")}</p>
          <h2 style={sectionTitle}>{pick(t("제휴 검증용 클릭 신호"), "Click signals for partnership validation")}</h2>
          <p style={bodyText}>
            {locale === "ko"
          ...
          <div style={{ display: "grid", gap: 8, marginTop: 14 }}>
            <button onClick={clearCommerceLog} disabled={careTotal === 0} style={{ ...dangerBtn, opacity: careTotal === 0 ? 0.5 : 1 }}>{pick(t("구매/상담 클릭 기록 삭제"), "Delete click log")}</button>
          </div>
        </section>

        <section style={noticeStyle}>
          <p style={sectionLabel}>{pick(t("의료적 경계"), "Medical boundary")}</p>
          <p style={bodyText}>
            {pick(
          ...
          <Link href="/scan" style={{ display: "block", textDecoration: "none" }}>
            <div style={{ position: "relative", padding: "15px 16px" }}>
              ...
              <div className="flex items-center justify-center" style={{ position: "relative", gap: 10 }}>
                <span style={{ fontFamily: "var(--font-hand)", fontSize: 24, color: "var(--ink)" }}>{pick(t("스캔으로 돌아가기"), "Back to scan")}</span>
                <span style={{ fontFamily: "var(--font-hand)", fontSize: 24, color: "var(--orange)" }}>→</span>
              </div>
            </div>
          </Link>
          <Link href="/care" style={{ ...outlineLink, display: "block", textAlign: "center", marginTop: 10 }}>{pick(t("구매/상담 연결"), "Shopping & clinic links")}</Link>
        </div>
      </div>
    </main>
  );
