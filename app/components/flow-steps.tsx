...
export function FlowSteps({ current }: { current: FlowKey }) {
  ...
  return (
    <nav aria-label={t("진행 단계")} style={{ display: "flex", alignItems: "center", gap: 7, margin: "2px 0 14px" }}>
      <Link href="/" aria-label={t("홈으로")} style={{ minWidth: "var(--tap-min)", minHeight: "var(--tap-min)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-hand)", fontSize: 22, lineHeight: 1, color: "var(--ink)", textDecoration: "none", paddingBottom: 2 }}>
        {t("아루")}
      </Link>
